...
namespace GlobalFlow {
    namespace DataProcessing {
        ...
        class SimpleVDF2DataReader : public DataReader {
        public:
            SimpleVDF2DataReader(int step) { stepMod = step; }
        ...
        private:
            ...
            void readInitialZetas(std::string pathZetas) {
                ...
                io::CSVReader<5, io::trim_chars<' ', '\t'>, io::no_quote_escape<','>> inZetas(pathZetas);
                inZetas.read_header(io::ignore_no_column, "spatID", "zeta0", "zeta1", "zeta2", "zeta3");
                while (inZetas.read_row(spatID, zeta0, zeta1, zeta2, zeta3)) {
                    int nodeID = 0;
                    try {
                        nodeID = lookupSpatIDtoID.at(spatID);
                    }
