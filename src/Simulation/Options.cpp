...
//TODO DON'T COMMIT OLD OPTIONS VERSION DONE BC. CONFIG.JSON NOT NEWEST VERSION WILL BE REWORKED (new options.cpp version commented below)
...
//BASE_PATH = input.get<string>("base_path");
            ...
            k_ghb_from_file = data_config.get<bool>("k_ocean_from_file");
            ...
            GHB_K = default_data.get<double>("oceanK");
            ...
            GHB_K_DIR = getOptional("ocean_conductance", data);
            ...
            if (mappings) {
                NODEID_SPATID = mappings.value().get<string>("nodeID-arcID");
            }
        }
